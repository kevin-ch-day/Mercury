...
def test_import_stream_strips_source_database_directives_and_definers(tmp_path: Path) -> None:
    ...
    payload = "\n".join(
        [
            ...
            "DROP DATABASE `android_permission_intel`;",
            "ALTER DATABASE `erebus_threat_intel_prod` CHARACTER SET utf8mb4;",
...
""",
    ...
    assert "DROP DATABASE" not in written
    assert "ALTER DATABASE" not in written
