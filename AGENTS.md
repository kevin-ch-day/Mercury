...
After cutover, routine writers target the primary HDD (`active_write_role=primary`). Prefer `MERCURY_PRIMARY_MOUNT` / `MERCURY_LEGACY_MOUNT` over deprecated `MERCURY_USB_MOUNT`. Legacy USB archive commands: `storage archive-status`, `archive-receipt`, `archive-remount-ro`. Backup storage lifecycle: Main Menu → Backup storage [4].
## Code conventions
...
- [docs/ops/](docs/ops/) — Track A acceptance, post-cutover storage, operator console, Erebus capture
